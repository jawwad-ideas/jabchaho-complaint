...
@section('content')
    <style>
        @media (max-width: 576px) {
            ...
            .order-action-btns>div {
                width: 48%;
                /* Two per row with some gap */
            }

            .order-action-btns .btn {
                width: 100%;
                border-radius: 8px;
                /* rectangle with slight round */
                padding: 12px;
                justify-content: center;
                text-align: center;
                white-space: normal;
        ...
        <div class="text-xl-start text-md-center text-center mt-xl-0 mt-3">
            <div class="btn-group order-action-btns d-flex flex-wrap justify-content-center" role="group">
                ...
                <div class="mb-3 order-action-button-div dropdown">
                    <button
                        class="btn btn-sm rounded bg-theme-dark-300 text-light me-2 border-0 fw-bold d-flex align-items-center p-2 gap-2 dropdown-toggle"
                    ...
                    <ul class="dropdown-menu" aria-labelledby="orderActionDropdown">
                        ...
                        <li>
                            <a href="#" id="syncorder" class="dropdown-item"
                                data-laundry-order-id="{{ $order->order_id }}">
                                Re-Sync Order
                            </a>
                        </li>
                            ...
                            @if ($sendBeforeEmail)
                                <li>
                                    <a class="dropdown-item sendWhatsApp" href="#" id="before_whatsapp"
                                        data-order-id="{{ $order->id }}" data-order-number="{{ $order->order_id }}"
                    ...
                    <div class="mb-3 complete-button-div dropdown">
                        ...
                        <ul class="dropdown-menu" aria-labelledby="emailDropdown">
                            @if ($sendBeforeEmail)
                                <li>
                                    <a class="dropdown-item sendEmailBeforeWashBtn" href="#"
                                        id="sendEmailBeforeWashBtn" data-order-id="{{ $order->id }}"
                            ...
                            @if ($sendFinalEmail)
                                <li>
                                    <a class="dropdown-item sendEmailBeforeWashBtn" href="#"
                                        data-order-id="{{ $order->id }}" data-email-type="final_email">
                                        {{ $sendFinalEmailTitle }}
                                    </a>
                                </li>
                            @endif
    ...
    <div class="page-content bg-white p-lg-5 px-2">
                ...
                <div class="col-lg-12">
                    ...
                    <div class="form-section mb-5">
                        ...
                        <div class="d-flex justify-content-end align-items-center">
                            <input type="text" class="form-control me-3" id="barcode" name="barcode"
                                placeholder="Barcode" autocomplete="off" style="max-width: 300px;">
                            <input type="button" value="Search"
                                class="btn bg-theme-yellow text-dark d-inline-flex align-items-center gap-3"
                                onclick="scrollToBarcode()">
                        </div>
                    </div>
                    ...
                    <div class="container mt-4 p-0">
                        <div class="order-basic-info mb-3">
                            ...
                            <div class="customer-basic-detail">
                                <div
                                    class="d-xl-flex d-lg-flex d-md-block cust-inr-detail-bar px-3 align-items-center gap-2">
                                ...
                                <div
                                    ...
                                    <h6 class="inr-vl">
                                        @if (!empty($order->location_type))
                                            {{ config('constants.laundry_location_type.store') }}
                                        @else
                                            {{ config('constants.laundry_location_type.facility') }}
                                        @endif
                                    </h6>
                                </div>
                            </div>

                            <input value="{{ $order->order_id }}" type="hidden" class="form-control"
                                name="order_number" placeholder="Order Number" readonly>
                        ...
                        <div class="mb-3">
                            ...
                            <input value="" type="file" class="form-control" name="remarks_attachment"
                                placeholder="" accept="image/*" capture="environment">
                            @if ($order->attachments)
                                ...
                                <a href="{{ $attachmentMainImage }}" target="_blank">
                                    <img class="order-img order-attachment-img img-thumbnail mt-3"
                                        src="{{ $attachmentThumbnail }}" alt="{{ $order->attachments }}"
                        ...
                        <div class="mb-3">
                            <button type="submit" id="UpdateOrderBtn"
                                class="btn bg-theme-yellow text-dark d-inline-flex align-items-center gap-3"> Update Order
                            </button>
                            <a href="{{ route('orders.index') }}/{{ $order->status }}"
                                class="btn bg-theme-dark-300 text-light">Back</a>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div class="scroll-to-scanner d-xl-none d-lg-none d-md-block d-sm-block">
            <button onclick="scrollToScanner()" class="btn btn-sm text-white p-2 "><i
        ...
        $(document).ready(function() {
            ...
            $("#startScanner").click(function() {
                const scannerContainer =
                    "scanner-container"; // Use the ID of the div, not the element itself
    ...
    <script>
        $(document).on('click', '#syncorder', function(event) {
            ...
            const laundryOrderId = $(this).data('laundry-order-id');

            if (!laundryOrderId) {
                alert('Laundry Order ID not found.');
                return;
            }
            ...
            const url = "{{ route('orders.sync.single') }}"; // <-- route name yahan
            ...
            $.ajax({
                ...
                success: function(response) {
                    if (response.success) {
                        ...
                        alert(response.message ?? 'Failed to sync order. Please try again.');
                    }
                },
                error: function(xhr) {
