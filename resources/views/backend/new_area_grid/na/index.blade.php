...
@section('content')
    <div
        ...
        <div class="p-title">
            <h3 class="fw-bold text-white m-0">National Assemblies</h3>
            <small class="text-white">Manage your National Assemblies here.</small>
        </div>
        <div class="text-xl-start text-md-center text-center mt-xl-0 mt-3">
            <div class="btn-group" role="group">
                <a href="{{ route('na.form') }}" class="text-decoration-none">
                    <small type="button"
                        class="btn btn-sm rounded bg-theme-green-light me-2 border-0 text-theme-green fw-bold d-flex align-items-center p-2 gap-2"><i
                            class="fa fa-solid fa-file-circle-plus"></i><span>Add National Assembly</span></small>
                </a>
    ...
    <div class="page-content bg-white p-lg-5 px-2">
        <div class="bg-light p-4 rounded">
            <div class="" id="filterBox" style="display:none;">
                <form class="form-inline" method="GET" action="{{ route('na.index') }}">
                    <div class="row mb-3">
            ...
            <div class="table-scroll-hr">
                <div class="d-flex my-2">
                    Showing results {{ ($nAs->currentPage() - 1) * config('constants.per_page') + 1 }} to
                    {{ min($nAs->currentPage() * config('constants.per_page'), $nAs->total()) }} of {{ $nAs->total() }}
                </div>
                <table class="table table-bordered table-striped table-compact">
                    ...
                    @foreach ($nAs as $key => $nA)
                        <tr>
                            <td>{{ Arr::get($nA, 'id') }}</td>
                            <td>{{ Arr::get($nA, 'name') }}</td>
                            @if (Auth::user()->can('na.edit'))
                                <td>
                                    <a class="btn btn-success btn-sm" href="{{ route('na.edit', $nA->id) }}">Edit</a>
                                </td>
                            @endif
                            @if (Auth::user()->can('na.destroy'))
                                <td>
                                    {!! Form::open([
                                        'method' => 'DELETE',
                                        'route' => ['na.destroy', $nA->id],
            ...
            <div class="d-flex">
                {!! $nAs->appends(Request::except('page'))->render() !!}
            </div>
