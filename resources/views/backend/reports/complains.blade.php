    ...
    <div class="page-content bg-white p-lg-5 px-2">
        <div class="container">
            <div class="" id="filterBox" style="display:none;">
                ...
                <form autocomplete="off" id="complainReport" action="{{ route('report-complains') }}" method="GET">
                    @csrf
                    ...
                    <div class="form-row row">
                        ...
                        <div class="form-group  mb-3 col-md-3">
                            <h6 class="fw-bold" for="period">Period:</h6>
                            <select class="mySelect form-control" id="period" name="period"> 
                        ...
                        <div class="form-group  mb-3 col-md-3">
                            ...
                            <input type="text" class="form-control p-2" autocomplete="off" id="cnic" name="cnic"
                                value="{{ request('cnic') ?? '' }}" placeholder="CNIC #" maxlength="15"
                                onpaste="return false;" onkeydown="return isNumberKey(this);" oninput="formatCNIC(this);">
                    ...
                    <div class="form-row row">
                        ...
                        <div class="form-group  mb-3 col-md-3">
                            ...
                            <input type="text" class="form-control p-2" autocomplete="off" name="mobile_number"
                                value="{{ request('mobile_number') ?? '' }}" onpaste="return false;"
                                placeholder="Mobile No." maxlength="11" onkeydown="return isNumberKey(event);">
    ...
    @endforeach
                                ...
                                @if (!empty($levelOne))
                                    @foreach ($levelOne as $row)
                                        <option value="{{ trim(Arr::get($row, 'id')) }}"
                                            {{ in_array(trim(Arr::get($row, 'id')), request('level_one', [])) ? 'selected' : '' }}>
                                            {{ trim(Arr::get($row, 'name')) }}
                                        </option>
                                    @endforeach
                                @endif
                            </select>
                            <input type="hidden" name="selected_level_one"
                                value="{{ implode(',', request('level_one', [])) }}">
                        </div>

                        <div class="form-group  mb-3 col-md-3 ">
                            <h6 class="fw-bold" for="type">Level Two:</h6>
                            <select class="mySelect form-control form-control-sm subCategories" id="level_two"
                                name="level_two[]" multiple="multiple">
                                <option value=''>--Select--</option>
                                @if (!empty($levelTwo))
                                    @foreach ($levelTwo as $row)
                                        <option value="{{ trim(Arr::get($row, 'id')) }}"
                                            {{ in_array(trim(Arr::get($row, 'id')), request('level_two', [])) ? 'selected' : '' }}>
                                            {{ trim(Arr::get($row, 'name')) }}
                                        </option>
                                    @endforeach
                                @endif
                            </select>
                            <input type="hidden" name="selected_level_two"
                                value="{{ implode(',', request('level_two', [])) }}">
                        </div>

                        <div class="form-group  mb-3 col-md-3">
                            <h6 class="fw-bold" for="type">Level Three:</h6>
                            <select class="mySelect form-control form-control-sm subCategories" id="level_three"
                                name="level_three[]" multiple="multiple"
                                ...
                                @if (!empty($levelThree))
                                    @foreach ($levelThree as $row)
                                        <option value="{{ trim(Arr::get($row, 'id')) }}"
                                            {{ in_array(trim(Arr::get($row, 'id')), request('level_three', [])) ? 'selected' : '' }}>
                                            {{ trim(Arr::get($row, 'name')) }}
                                        </option>
                                    @endforeach
                                @endif
                            </select>
                            <input type="hidden" name="selected_level_three"
                                value="{{ implode(',', request('level_three', [])) }}">
                        </div>
                            ...
                            <select class="mySelect form-control p-2" id="complaint_status_id" name="complaint_status_id">
                                ...
                                @if (!empty($statuses))
                                    @foreach ($statuses as $row)
                                        <option value="{{ trim(Arr::get($row, 'id')) }}"
                                            {{ request('complaint_status_id') == trim(Arr::get($row, 'id')) ? 'selected' : '' }}>
                                            {{ trim(Arr::get($row, 'name')) }}
                                        </option>
                                    @endforeach
                                @endif
                        ...
                        <div class="form-group  mb-3 col-md-3">
                            ...
                            <select class="mySelect form-control form-control-sm" name="complaint_approved_id">
                                <option value="">--Select--</option>
                                <option value="1" {{ request('complaint_approved_id') == '1' ? 'selected' : '' }}>
                                    Approved</option>
                                <option value="0" {{ request('complaint_approved_id') == '0' ? 'selected' : '' }}>
                                    Pending</option>
                            </select>
                        </div>
        ...
        <div class="container mt-5">
            ...
            <div class="text-end">
                <div class="btn-group" role="group">
                    @if (!empty($counts))
                        <form action="{{ route('report-complains') }}" method="GET">
                            ...
                            <input type="hidden" name="level_one"
                                value="{{ is_array(request('level_one')) ? implode(',', request('level_one')) : request('level_one') }}">
                            <input type="hidden" name="level_two"
                                value="{{ is_array(request('level_two')) ? implode(',', request('level_two')) : request('level_two') }}">
                            <input type="hidden" name="level_three"
                                value="{{ is_array(request('level_three')) ? implode(',', request('level_three')) : request('level_three') }}">
                            ...
                            <input type="hidden" name="provincial_assembly_id"
                                value="{{ request('provincial_assembly_id') }}">
                            <input type="hidden" name="national_assembly_id"
                                value="{{ request('national_assembly_id') }}">
                            <input type="hidden" name="complaint_status_id"
                                value="{{ request('complaint_status_id') }}">
                            <input type="hidden" name="mpa_id" value="{{ request('mpa_id') }}">
                            <input type="hidden" name="mna_id" value="{{ request('mna_id') }}">
                            <input type="hidden" name="complaint_approved_id"
                                value="{{ request('complaint_approved_id') }}">
                ...
                <table class="table table-bordered table-striped table-compact">
                    <thead>
                        <tr>
                            ...
                            <th>Karachi Complaints</th>
                            <th>Hyderabad Complaints</th>
                        </tr>
                    </thead>
                    @if (!empty($counts))
                        <tbody id="results-report">
                            @foreach ($counts as $count)
                                <tr>
                                    <td>{{ $count->total }}</td>
                                    <td>{{ $count->complaint_registered }}</td>
                                    <td>{{ $count->in_process }}</td>
                                    <td>{{ $count->hold }}</td>
                                    <td>{{ $count->resolved }}</td>
                                    <td>{{ $count->closed }}</td>
                                    <td>{{ $count->approved }}</td>
                                    <td>{{ $count->pending_approval }}</td>
                                    <td>{{ $count->karachi_complaints }}</td>
                                    <td>{{ $count->hyderabad_complaints }}</td>
